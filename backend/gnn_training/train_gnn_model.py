...
class GNNTrainer:
    ...
    def generate_cascade_labels(self) -> None:
        ...
        high_crit_idx = self.nodes_df['CASCADE_RISK_SCORE'].nlargest(
            self.config.num_cascade_seeds
        ).index.tolist()
        ...
        total = len(self.y)
        print(
            f"  Cascade labels: {positive_count} affected / {total} total "
            f"({100*positive_count/total:.1f}%)"
        )
