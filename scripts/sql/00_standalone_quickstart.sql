...
-- Note: If you want a different database name, change 'FLUX_DB' below
CREATE DATABASE IF NOT EXISTS FLUX_DB
    DATA_RETENTION_TIME_IN_DAYS = 7
    COMMENT = 'Flux Operations Center - Grid Analytics & Visualization';

USE DATABASE FLUX_DB;

-- Note: If you want a different warehouse name, change 'FLUX_WH' below
CREATE WAREHOUSE IF NOT EXISTS FLUX_WH
...
-- Topology Feeders View (columns must match backend server_fastapi.py _fetch_feeders query)
...
SELECT 
    ...
    ROUND(30 + UNIFORM(0::FLOAT, 60::FLOAT, RANDOM()), 2) AS LOAD_UTILIZATION_PCT,
    c.CIRCUIT_ID,
    tm.CAPACITY_KVA AS RATED_KVA,
    ROUND(HAVERSINE(s.LATITUDE, s.LONGITUDE, tm.LATITUDE, tm.LONGITUDE), 3) AS DISTANCE_KM,
...
JOIN PRODUCTION.SUBSTATIONS s ON tm.SUBSTATION_ID = s.SUBSTATION_ID
LEFT JOIN PRODUCTION.CIRCUIT_METADATA c ON c.SUBSTATION_ID = tm.SUBSTATION_ID
...
-- Service Areas (columns must match backend server_fastapi.py fetch_service_areas query)
CREATE OR REPLACE VIEW FLUX_OPS_CENTER_SERVICE_AREAS_MV AS
SELECT 
    c.CIRCUIT_ID,
    c.SUBSTATION_ID,
    s.SUBSTATION_NAME,
    s.LATITUDE AS CIRCUIT_CENTER_LAT,
    s.LONGITUDE AS CIRCUIT_CENTER_LON,
    ROUND(40 + UNIFORM(0::FLOAT, 50::FLOAT, RANDOM()), 2) AS AVG_LOAD_UTILIZATION_PCT,
    ROUND(70 + UNIFORM(0::FLOAT, 25::FLOAT, RANDOM()), 2) AS AVG_HEALTH_INDEX
FROM PRODUCTION.CIRCUIT_METADATA c
JOIN PRODUCTION.SUBSTATIONS s ON c.SUBSTATION_ID = s.SUBSTATION_ID;
...
CREATE TABLE IF NOT EXISTS GRID_NODES (
    ...
    PARENT_NODE_ID VARCHAR(50),  -- Hierarchical link for graph traversal
    SUBSTATION_ID VARCHAR(50),
    -- GNN Features
...
INSERT INTO GRID_NODES (NODE_ID, NODE_TYPE, NODE_NAME, LATITUDE, LONGITUDE, LAT, LON, VOLTAGE_LEVEL, VOLTAGE_KV, CAPACITY_KVA, CAPACITY_KW, PARENT_NODE_ID, SUBSTATION_ID, DEGREE_CENTRALITY, BETWEENNESS_CENTRALITY, LOAD_FACTOR, AGE_YEARS, HEALTH_SCORE, CRITICALITY_SCORE, DOWNSTREAM_TRANSFORMERS, DOWNSTREAM_CAPACITY_KVA)
SELECT 
    TRANSFORMER_ID,
    'TRANSFORMER',
    TRANSFORMER_NAME,
    ...
    COALESCE(PRIMARY_VOLTAGE_KV, 12.47),  -- VOLTAGE_KV numeric
    CAPACITY_KVA,
    CAPACITY_KVA,  -- CAPACITY_KW (same as KVA for transformers)
    SUBSTATION_ID,  -- PARENT_NODE_ID (transformers connect to substations)
    SUBSTATION_ID,
    ROUND(RANDOM() * 0.5 + 0.1, 4),
    ROUND(RANDOM() * 0.3, 4),
    ROUND(RANDOM() * 0.8 + 0.2, 3),
    ROUND(5 + RANDOM() * 35, 1),  -- AGE_YEARS (5-40 years)
    HEALTH_SCORE,
    ROUND(RANDOM() * 0.8 + 0.1, 3),  -- CRITICALITY_SCORE
    0,  -- DOWNSTREAM_TRANSFORMERS (transformers are leaf nodes)
    0   -- DOWNSTREAM_CAPACITY_KVA
FROM PRODUCTION.TRANSFORMER_METADATA
...
SELECT 
    ...
    s.LATITUDE,
    s.LONGITUDE,
    s.LATITUDE,  -- LAT alias
    s.LONGITUDE,  -- LON alias
    s.VOLTAGE_CLASS,
    CASE 
        ...
        ELSE 12.47
    END,  -- VOLTAGE_KV numeric
    s.CAPACITY_MVA * 1000,
    s.CAPACITY_MVA * 1000,  -- CAPACITY_KW
    NULL,  -- PARENT_NODE_ID (substations are root nodes)
    s.SUBSTATION_ID,
    ROUND(RANDOM() * 0.3 + 0.5, 4),
    ROUND(RANDOM() * 0.5 + 0.2, 4),
    ROUND(RANDOM() * 0.6 + 0.3, 3),
    ROUND(10 + RANDOM() * 50, 1),  -- AGE_YEARS (10-60 years for substations)
    ROUND(85 + RANDOM() * 15, 1),
    ROUND(RANDOM() * 0.5 + 0.4, 3),  -- CRITICALITY_SCORE (higher for substations)
    COALESCE(t_count.cnt, 0),  -- DOWNSTREAM_TRANSFORMERS
    COALESCE(t_count.total_capacity, 0)  -- DOWNSTREAM_CAPACITY_KVA
FROM PRODUCTION.SUBSTATIONS s
LEFT JOIN (
...
INSERT INTO GRID_EDGES (EDGE_ID, FROM_NODE_ID, TO_NODE_ID, EDGE_TYPE, LENGTH_METERS, DISTANCE_KM, IMPEDANCE_PU, VOLTAGE_LEVEL, CAPACITY_AMPS, IS_OPEN)
SELECT 
    'EDGE_' || ROW_NUMBER() OVER (ORDER BY t.TRANSFORMER_ID),
    t.SUBSTATION_ID,
    t.TRANSFORMER_ID,
    'PRIMARY_FEED',
    ROUND(500 + RANDOM() * 5000, 1) AS LENGTH_METERS,
    ROUND((500 + RANDOM() * 5000) / 1000, 3) AS DISTANCE_KM,  -- Convert to km
    ROUND(0.01 + RANDOM() * 0.05, 4) AS IMPEDANCE_PU,
    COALESCE(t.PRIMARY_VOLTAGE_KV::VARCHAR || 'KV', '12KV') AS VOLTAGE_LEVEL,
...
INSERT INTO NODE_CENTRALITY_FEATURES_V2 (NODE_ID, DEGREE_CENTRALITY, BETWEENNESS_CENTRALITY, CLOSENESS_CENTRALITY, PAGERANK_SCORE, PAGERANK, EIGENVECTOR_CENTRALITY, CASCADE_IMPACT_SCORE, VULNERABILITY_SCORE, CASCADE_RISK_SCORE_NORMALIZED, TOTAL_REACH, NEIGHBORS_1HOP, NEIGHBORS_2HOP, CRITICALITY_RANK, NODE_TYPE, SUBSTATION_ID)
SELECT 
    NODE_ID,
    DEGREE_CENTRALITY,
    BETWEENNESS_CENTRALITY,
    ROUND(RANDOM() * 0.4 + 0.3, 4),
    ROUND(RANDOM() * 0.02 + 0.001, 6),  -- PAGERANK_SCORE
    ROUND(RANDOM() * 0.02 + 0.001, 6),  -- PAGERANK (alias)
    ROUND(RANDOM() * 0.5 + 0.1, 4),  -- EIGENVECTOR_CENTRALITY
    ROUND(RANDOM() * 100, 2),
    ROUND(RANDOM() * 0.8 + 0.1, 3),
    ROUND(RANDOM() * 0.9 + 0.1, 4),
    FLOOR(RANDOM() * 50 + 5)::INT,  -- TOTAL_REACH
    FLOOR(RANDOM() * 10 + 1)::INT,  -- NEIGHBORS_1HOP
    FLOOR(RANDOM() * 30 + 5)::INT,  -- NEIGHBORS_2HOP
...
WITH numbered_substations AS (
    SELECT 
        s.*,
        ROW_NUMBER() OVER (ORDER BY s.SUBSTATION_ID) AS rn
    FROM PRODUCTION.SUBSTATIONS s
)
SELECT 
    'CASCADE_' || rn,
    ...
    SUBSTATION_NAME,
    'SUBSTATION',
    FLOOR(5 + UNIFORM(0::FLOAT, 20::FLOAT, RANDOM()))::INT,
    FLOOR(1000 + UNIFORM(0::FLOAT, 50000::FLOAT, RANDOM()))::INT,
    ROUND(CAPACITY_MVA * (0.3 + UNIFORM(0::FLOAT, 0.5::FLOAT, RANDOM())), 2),
    PARSE_JSON('[{"node": "' || SUBSTATION_ID || '", "depth": 0}, {"node": "downstream", "depth": 1}]'),
    FLOOR(2 + UNIFORM(0::FLOAT, 4::FLOAT, RANDOM()))::INT,
    CASE MOD(rn, 3) 
        WHEN 0 THEN 'PEAK_SUMMER'
        WHEN 1 THEN 'STORM_EVENT'
        ELSE 'EQUIPMENT_FAILURE'
    END
FROM numbered_substations
...
-- Show Postgres instance details and capture host using flow operator
SHOW POSTGRES INSTANCES LIKE 'FLUX_OPS_POSTGRES'
->>
